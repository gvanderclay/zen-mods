...
- A durable design choice gets a record in `notes/keep-loaded/DECISIONS.md` in the
  checkpoint that makes it. Accepted records are superseded, never rewritten.
...
`notes/keep-loaded/` is gitignored: the roadmap and the ledger are local working
notes, not part of what this repository publishes. Both are load-bearing anyway.

- `notes/keep-loaded/PLAN.md` — checkpoints and their order
- `notes/keep-loaded/DECISIONS.md` — what was decided, why, and against which source
